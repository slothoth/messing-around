	...
	for commandRow in GameInfo.UnitCommands() do
		if ( commandRow.VisibleInUI ) then
			...
			if (actionHash == UnitCommandTypes.ENTER_FORMATION) then
				...
				local bCanStart, tResults = UnitManager.CanStartCommand( pUnit, actionHash, nil, true); -- here?
				if (bCanStart and tResults) then
					if (tResults[UnitCommandResults.UNITS] ~= nil and #tResults[UnitCommandResults.UNITS] ~= 0) then
						local tUnits = tResults[UnitCommandResults.UNITS]; -- SLTH where is this UnitCommandResults coming from
						print('---------')
						-- = Units.GetUnitsInPlot(28,10)
						for i, unit in ipairs(Players[0]:GetUnits()) do
							print('checking unit:')
